# 1조 무선네트워크 프로젝트
김민규(MMMMins/조장): 프로젝트 틀 작성 및 라즈베리, 백엔드, 프론트간 논리적 소켓 연결 구현, 전체적인 백엔드 로직작성   
한용준(HANYONUJUN): ui 웹페이지 디자인 및 구조, 기능 작업   
서예령(yeryeong0519): 문자전송 기능 및 문자전송API제작, 쓰러짐감지시 문자전송 로직작성    
신건호(geonhooo): 스트리밍 시작 버튼, API를 이용하여 감지 로그 받아오기 및 화면 출력   
이정우(wjddn8038):  쓰러짐 감지모델을 만드는 역할로, 다양한 쓰러짐 데이터랑 쓰러지지 않은 데이터를 학습   
김건우(inhatckgw): 라즈베리파이 <-> 백엔드간 통신 기초 코드 작성 및 프론트에서 백엔드로 데이터 전달시 데이터 파싱 구현
