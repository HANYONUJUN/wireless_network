...
async def process_image_async(img, model_path, output_path):
    loop = asyncio.get_event_loop()

    # 비동기로 모델 처리를 수행
    ai_img = await loop.run_in_executor(None, process_image, img, model_path, output_path)

    return ai_img

async def handle_image_data(websocket: WebSocket, data_json):
...
async def websocket_imagedata(websocket: WebSocket):
    try:
        while True:
            ...
            # 이미지 데이터 처리를 비동기로 호출
            await handle_image_data(websocket, data_json)
